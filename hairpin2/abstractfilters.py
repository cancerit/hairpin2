...
from typing import ClassVar, Protocol, override, Generic, TypeVar, final, Any
...
class FilterResult(ABC, Generic[CodeEnum_T]):
    ...
    def __post_init__(self) -> None:
        if self.flag and not isinstance(self.flag, bool):
            raise ValueError('The flag field may only be set to a boolean value or None')
        if self.code and not isinstance(self.code, self.Codes):
            raise ValueError('The code field may only be filled by values given in the associated CodeEnum ClassVar, or None')
        if self.flag and not self.code:
...
class FilterTester(ABC, Generic[ReadCollection_T, FilterParams_T, FilterResult_T]):
    ...
    This method enforces a signature which:
        - defines a common interface to filter tests
        - respects the "Livkov Substitution Principle", so you don't need to know about the implementation details of a FilterTester to be able to use the `test()` method
    """
    @classmethod
    @abstractmethod
    def test(
        cls,
        variant_reads: ReadCollection_T,
        params: FilterParams_T
...
@final
@dataclass(slots=True, frozen=True)
class ALResult(FilterResult[ALCodes]):
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.alt, str):
            raise ValueError('The alt field may only be represented by a str')
        if self.avg_as and not isinstance(self.avg_as, float):
            raise ValueError("average AS may only be represented by a float")
...
@final
@dataclass
class ALParams(IsDataclass):
    alt: str
    al_thresh: float = 0.93
@final
@dataclass
class ALFilter(FilterTester[Sequence[AlignedSegment], ALParams, ALResult]):  # n.b. you can retain the broadest generic nature by reusing the TypeVar and not specifying a concrete type for any of the type arguments to FilterTester
    """
    Describe filter
    """
    @classmethod
    ...
    def test(
        ...
        if len(aln_scores) != 0:
            ...
            flag = False
            if avg_as <= params.al_thresh:
                flag = True
            fresult = ALResult(flag, code, params.alt, avg_as)
        else:
            code = ALResult.Codes.INSUFFICIENT_READS
            flag = False
            fresult = ALResult(flag, code, params.alt, None)
